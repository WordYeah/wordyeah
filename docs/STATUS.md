...
- [CX] 2026-08-09 质量预标已排空：冻结主审批次 1,100/1,100 条均有 AI 建议，实际完成 1,100 次 `vision_review_1` 与 127 次 `vision_review_2`；活动任务、重复成功、attempt 复用、失联租约和未关联结果均为 0。人工主审、固定 10% 双审和仲裁仍为 0，质量报告继续 `INCOMPLETE`，未把 AI 建议计入 ground truth。
- [CX] 2026-08-09 普通旧队列的 27 条完整性绑定替代任务已由本机视觉链排空，其中 24 条内部自动通过、3 条进入 `human_required`，7 条经过独立二审；普通 lane 无 queued/running job，质量决定仍为 0，未写生产头像。G2A Web 池只读水位为 Web available 16,400、Build available 0；`grok-chat-fast` 合成图真实 canary 返回 `allow/1.0`，耗时 7.25 秒，未据此宣称 Build 池可用。
- [CX] 2026-08-09 浏览器隔离验收已按现代码重跑：源 SQLite 仅以 `mode=ro + query_only` 备份，临时副本未产生审核写入；九个页面、1440/1280/1180/1024/900/760/640/390 八档宽度的 184 个下拉实例、质量分页、盲 AI 建议、键盘操作和三角色独立 reviewer session 均通过。聚合验收除代表性 corpus 的人工真值缺失外全部 PASS，`enforce=false`。
- [CX] 2026-08-09 已补充长期开发维护约定和在线数据接入运行手册，明确模块职责、变更分类、验证矩阵、发布回滚、日常维护、1,000/10,000/100,000/全量 Shadow 放量、自动停止和独立生产写回门槛；这些文档不构成生产扫描或写回授权。
- [CX] 2026-08-09 审核台删除系统健康阶段卡和彩质人工规则中的装饰性彩色边线；列表首尾增加 12px 留白、行内垂直间距增大并修正搜索图标的绝对居中基线。
- [CX] 2026-08-09 UI 修正后隔离浏览器验收使用只读 corpus 数据库副本重跑为 PASS，`source_database_mutated=false`、`production_avatar_write=false`；历史页验收 selector 已与统一移动工作区组件同步。
...
- [CX] 2026-08-20 首批 1,000 行登记表 canary 的 960 个唯一内容已全部到达审核终态：自动通过 864、自动拒绝 3、人工通过 92、人工拒绝 1；G2A 一审成功 302、本机 Ollama 一审成功 658、Ollama 二审成功 160，活动任务 0，调和完成 960/960、失败 0，SQLite integrity 为 `ok`，错误路径产生 allow 为 0。只读终态证据保存在仓库外 `registry-canary-terminal-evidence-20260820.json`，未写生产头像。审核队列中的人工操作与冻结质量 corpus 的独立真值标注是两套账本；现有普通审核不能代替仍为 0/1,100 的质量主审，因此整体质量门闸继续 `INCOMPLETE`。
- [CX] 2026-08-20 人工黑名单写回候选已接入但默认关闭：只允许 `cravatar` 工作区的已终审 blacklist 项、固定 Cravatar bans 路径和公网 HTTPS 443，禁止跨工作区、非白名单主机、URL 凭据/query/fragment、重定向和上游正文回显；请求带内容身份与幂等键。运行时未配置 `CRAVATAR_BAN_URL`/`CRAVATAR_BAN_TOKEN`，隔离 Chromium 验收仍为 `production_avatar_write=false`。完整验证为 323 个 pytest、12 个 subtest、Ruff、compileall 和 diff check PASS；唯一警告为 Starlette/httpx 弃用提示。
