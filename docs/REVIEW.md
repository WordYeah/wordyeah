...
- 风险、审计条件和每页数量使用同一套站内下拉菜单，不调用系统原生选择器；展开层固定贴合触发器，并在 1440–390 px 断点内保持可见。
...
- 质量 corpus 的常规主审和双审只在质量页处理，不占用运营审核队列；只有明确进入仲裁或自动链路升级为 `human_required` 的项目才进入运营人工队列。

### 私有 corpus 的 AI 预标注

代表性 corpus 可以先进入与普通头像相同的高级视觉链，质量页把模型输出显示为
“AI 建议”，用于提交人工结论后的复核与分歧定位。为避免人工标签被模型答案锚定，每位
reviewer 在提交自己的独立结论前只看到“盲审封存”，提交后才显示 AI 建议；双审样本的
第二位 reviewer 同时看不到第一位结论。AI 预标注和人工真值是两条独立记录：预标注不会写入
...
普通常驻高级视觉 worker 必须使用
`--vision-exclude-context-marker quality_ai_prelabel=true`。这样冻结质量建议 lane 不会被
普通审核进程抢占；只有上面的有界 corpus 命令可以包含该 marker。
