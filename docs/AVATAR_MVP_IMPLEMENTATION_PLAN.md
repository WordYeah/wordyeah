...
- 全量测试：208 passed，12 subtests passed；仅有 Starlette/httpx 弃用警告。
...
- 10% 双审选择：已按固定 seed 和全部源样本指纹冻结 110 条，五分层配额为 30/30/10/20/20；输出拒绝覆盖不同结果。当前 `dual_review_completed=0`，因此只算任务集合已冻结，不算真实双审或评测集完成。
