...
状态说明：P0-P4 的核心代码和自动化测试已实现；P5 只有短时吞吐 smoke 与浏览器主路径验收，代表性 corpus、15 分钟压测和生产级 shadow 调度仍未完成。
...
- 全量测试：162 passed，12 subtests passed；仅有 Starlette/httpx 弃用警告。
- 浏览器：1280×800 验证工作区切换、列表和高风险禁批；390×844 验证移动工作区菜单、队列与单项审核，决定动作不弹确认框。
- 队列短时 smoke：50 jobs/s、25 个任务、约 49.84 jobs/s，状态为 `SMOKE`；因未运行 15 分钟，不判 PASS。
