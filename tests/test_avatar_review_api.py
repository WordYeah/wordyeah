    ...
    def test_queue_views_and_bounded_batch_review(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with TestClient(app) as client:
                ...
                self.assertIn('class="per-page-dropdown menu-select"', queue_page.text)
    ...
    def test_login_review_and_optimistic_action(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with TestClient(app) as client:
                ...
                self.assertIn('class="tabs-header"', queue_page.text)
                self.assertIn('class="filter-controls"', queue_page.text)
                self.assertIn('class="risk-filter-dropdown menu-select"', queue_page.text)
                self.assertIn(".tabs-header {", queue_page.text)
