...
def _export(path: Path, *, url: str | None = None) -> None:
    email_hash = "a" * 32
    path.write_text(
        json.dumps(
            {
                ...
                "avatar_url": url or f"https://cravatar.cn/avatar/{email_hash}",
...
@pytest.mark.parametrize(
    "url",
    (
        "http://cravatar.cn/avatar/" + "a" * 32,
        "https://evil.example/avatar/" + "a" * 32,
        "https://cravatar.cn/avatar/" + "a" * 32 + "?x=1",
        "https://cravatar.cn/avatar/" + "c" * 32,
    ),
)
